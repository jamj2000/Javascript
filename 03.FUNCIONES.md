...
- [4. Valor devuelto o valor de retorno](#4-valor-devuelto-o-valor-de-retorno)
  - [4.1. Return implícito](#41-return-implícito)
...
(a, b) => a + b;                   // expresada en forma de función flecha, return implícito
```

> [!NOTE]
>
> Más información más abajo en la sección [4.1. Return implícito](#41-return-implícito) 
...
Si no hay sentencia `return` o aparece esta sentencia sin nigún valor devuelto.
...
sumar(3,5)   // Muestra La suma de 3 y 5 es 8. Devuelve undefined
```
...
## 4.1. Return implícito

En JavaScript, el **return implícito** ocurre cuando una función (normalmente una *arrow function*) devuelve un valor sin escribir explícitamente `return`.
...
Aquí no hay `return`, pero la función devuelve automáticamente a + b

Eso es el return implícito

**Cuándo funciona**

Solo ocurre cuando la *arrow function* tiene una expresión directa, sin llaves `{}`:

```js
const doble = x => x * 2;
```

**Cuándo NO hay return implícito**

Si usas llaves `{}`, necesitas `return`:
...
console.log(doble(4)); // undefined ❌
```

**Caso especial: devolver objetos**

Si quieres devolver un objeto con return implícito, debes envolverlo en paréntesis `()`:

```js
const crearUsuario = () => ({ nombre: "Ana", edad: 25 });
```

Si no:

```js
const crearUsuario = () => { nombre: "Ana" }; // ❌ error / undefined
```

> [!TIP] **Resumen rápido**
>
> - ✅ `() => expresion` → return implícito
> - ❌ `() => { ... }` → necesitas return
> - ⚠️ Objetos → usar ()
...
JavaScript es un lenguaje con tipado dinámico y débil, lo que hace que sus comprobaciones de tipos sean menos estrictas que en lenguajes fuertemente tipados. Esto puede a veces provocar errores en programadores que no conocen bien el lenguaje.
